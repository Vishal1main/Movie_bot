            ...
            if refData and refData.split("-", 1)[0] == "biisal":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        buttons = [[
                            ...
                            [
                            InlineKeyboardButton('🎁 ᴘʀᴇᴍɪᴜᴍ ғʀᴇᴇ 🎁', callback_data=f'point#{message.from_user.id}'),
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/TMR_movie_request_group/</code>")    
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/TMR_update_channel').json()
...
async def set_shortner_2(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/TMR_DEVELOPER').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
